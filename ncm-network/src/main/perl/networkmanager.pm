...
Readonly my $NMCLI_CONN => [qw(nmcli conn)];
Readonly my $NMCLI_DEV_STATUS => [qw(nmcli dev status)];
...
sub is_valid_interface
{
    ...
    # FIX ME: needs to put right regexp $DEVICE_REGEXP? do we need to for nmconnection i.e
    # if ($filename =~ m/$DEVICE_REGEXP/) {
    if ($filename =~ /(\w+\d+)\.(nmconnection)/) {
        # name and id kept same so value for both is same.
        my $ifupdownname = $1;
        my $name = $1;
...
sub file_dump
{
    ...
    if (!$self->file_exists($file) || $self->mk_bu($file, $testcfg)) {
        my $fh = CAF::FileWriter->new($testcfg, mode =>0400, log => $self, keeps_state => 1);
...
sub get_current_config
{
    ...
    $output .= "\nls -lrt $NMCFG_DIR\n";
    $output .= $self->runrun(['ls', '-ltr', $NMCFG_DIR]);
    ...
    $output .= "\n@$NMCLI_CONN\n";
    $output .= $self->runrun($NMCLI_CONN);
    
    $output .= "\n@$NMCLI_DEV_STATUS\n";
    $output .= $self->runrun($NMCLI_DEV_STATUS);
...
sub process_network
{
    ...
    foreach my $ifname (sort keys %{$nwtree->{interfaces}}) {
        ...
        # add ethtool options preparsed. These will be set in keyfile config
        # some are needed on boot (like autoneg/speed/duplex)
        # TODO: need add these to nmconnection keyfile 
        ...
        # Newly added used when generating the keyfile, 
        my $media = $nics->{$nicname}->{media};
        if ($media) {
            $iface->{media} = $media;
        ...
        if ($ifname =~ m/^vlan\d+/ && ! exists($self->{vlan})) {
            ...
            # TODO: keyfile generations for vlan interface. WARN for now.
            $self->warn("vlan interface not supported");
        }
...
sub gather_existing
{
    ...
    foreach my $filename (@$files) {
        ...
        $self->debug(3, "Found nmconnection $msg $file");
    }
...
# NOTE: currenly only single interface config and bonding is support. more to follow
# 
sub make_nm_keyfile { # arg1 = 'eth0'
    ...
    my $ethtool_config = $iface->{'ethtool_opts'};
    if (scalar $ethtool_config) {
        # TODO: testing ethtool opton is configured, WARN
        $self->warn("ethtool_opts found, but not supported.");
        push (@data, '[ethtool]');
    ...
    # TODO: can add ethtool options to keyfile, should look into this.
    # push(@data,'[ethtools]');
    return \@data;
}
...
# 
# NOT used for generatting inteface content, this is done using make_nm_keyfile().
sub make_ifcfg
{
...
# TODO: how to do this with NM keyfile format?
    ...
    foreach my $file (sort keys %$exifiles) {
        ...
        if ($valid) {
            ...
            if ($value == $NOCHANGES) {
                ...
                if ($ifaces->{$iface}->{master}) {
                ...
                } elsif ($file eq "$NMCFG_DIR/$iface.nmconnection" && $self->any_exists($file)) {
                    ...
                    # TODO: needs testing with NM keyfile format is this needed?
...
sub make_ifup
{
    ...
    foreach my $iface (sort keys %$ifdown) {
        ...
        if (exists($exifiles->{"$NMCFG_DIR/$iface.nmconnection"}) &&
            $exifiles->{"$NMCFG_DIR/$iface.nmconnection"} == $REMOVE) {
    ...
    if ($nwupdated || @ifaces) {
        if (@ifaces) {
            ...
            foreach my $iface (@ifaces) {
                # can do nmcli conn up $iface if required but I dont think its required?
...
# TODO: this can be added to keyfile?
