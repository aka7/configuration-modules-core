...
sub generate_dnf_modules {
    ...
    foreach my $name (sort keys %$modules) {
        ...
        if ($module_changed > 0 and !$CAF::Object::NoAction) {
            my ($cmd_exit, $cmd_out, $cmd_err) = $self->execute_command([DNF_MODULE_RESET, $name],
                "module stream change detected. Reset managed module stream $name for update", 1, "/dev/null", 1);
            if ($cmd_exit) {
